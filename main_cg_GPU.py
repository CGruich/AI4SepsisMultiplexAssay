...
def load_code(code_folder_path, verbose=True):
    ...
    for file_name in os.listdir(code_sample_folder):
        ...
        # Load region.
        region = cv2.imread(
            os.path.join(code_sample_folder, file_name), cv2.IMREAD_ANYDEPTH
        )
        try:
...
def sort_alphanumeric(string_list):
    assert hasattr(
        string_list, "sort"
    ), "ERROR! TYPE {} DOES NOT HAVE A SORT FUNCTION".format(type(string_list))
    """
...
def grid_search_region_classifier(
    ...
    if pipeline_inputs is not None:
        load_hpo_path = pipeline_inputs.get("grid_search_hpo", None).get(
            "hpo_file", None
        )
        ...
        cross_validation = pipeline_inputs.get("strat_kfold", None).get(
            "activate", None
        )
        k = pipeline_inputs.get("strat_kfold", None).get("num_folds", None)
        random_state = pipeline_inputs.get("strat_kfold", None).get(
            "random_state", None
        )
        save_every = pipeline_inputs.get("grid_search_hpo", None).get(
            "save_every", None
        )
        ...
        for row in tqdm(hpo_df.to_dict(orient="records")):
            ...
            if cross_validation:
                ...
                for key, value in scores.items():
                    if value == "ERR":
                        cross_validation_loss_df.loc[
                            len(cross_validation_loss_df)
                        ] = write_row
                        cross_validation_acc_df.loc[
                            len(cross_validation_acc_df)
                        ] = write_row
...
def classify_regions(pipeline_inputs: dict = None, load_path=None, img_folder=None):
                ...
                # If the file is a reference image,
                    ...
                    for image_name in filenames:
                        ...
                        if (
                            ...
                            hologram = cv2.imread(
                                "{}/{}".format(raw_code_dir, image_name),
                                cv2.IMREAD_ANYDEPTH,
                            )
                    ...
                    for hologram in holograms:
                        ...
                        (
                            positive_regions,
                            negative_regions,
                        ) = region_detector.detect_regions(
    ...
    # Legacy code
        ...
        for hologram in holograms:
            ...
            for i in range(len(positive_regions)):
                file_path = "data/classifier_training_samples/positive/{}_positive_{}.png".format(
                    folder_name, i + sum_pos
                )
                region = positive_regions[i]
                shape = region.shape
                new_shape = (*shape[1:], shape[0])
            ...
            for i in range(len(negative_regions)):
                file_path = "data/classifier_training_samples/negative/{}_negative_{}.png".format(
                    folder_name, i + sum_neg
                )
                region = negative_regions[i]
                shape = region.shape
                new_shape = (*shape[1:], shape[0])
...
def get_intensity(
    ...
    intensities = (
        [file_names]
        + [["x", "y", "intensity"] * int(int(len(file_names)) / 3)]
        + intensities
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--reg_path", type=str, default=None, help="load region detector path"
    )
    parser.add_argument(
        "--code_path",
        type=str,
        default="data/best/best_code_classifier.pt",
    ...
    if action == "find_mser_params":
        ...
        if pipeline_inputs is not None:
            ...
            def objective_with_custom_input(trial):
                return objective(trial, pipeline_inputs)
