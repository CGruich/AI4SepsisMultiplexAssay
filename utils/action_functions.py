...
def find_particle_intensities():
    ...
    for code_path in code_paths:
        ...
        ref = cv2.imread(os.path.join(full_path,"ref.tiff"), cv2.IMREAD_ANYDEPTH).astype(np.float32)
        ...
        for file_name in os.listdir(full_path):
            ...
            if ".json" in file_name:
                ...
                grayscale = helper_functions.normalize_by_reference(hologram, ref, scale_to_bit_depth=False, ref_already_convolved=True)
    ...
    for code, code_data in code_data.items():
        ...
        for intensity, estimates in intensity_dict.items():
            intensity_data.append((intensity, np.mean(estimates), np.std(estimates), len(estimates)))
        ...
        for arg in intensity_data:
            ...
            print("{}\t{}\t{}\t{}\t{}".format(code, intensity, avg_intensity_estimate, intensity_estimate_std, particle_count))
        print()
