...
def generate_grid_search_file(choice_list, choice_labels, choice_type, hyperparameter_lists, save_path = os.getcwd()):
    ...
    for choice_index in range(len(choice_list)):
        ...
        while choice_list[choice_index] != "DONE":
            # Enter a value
            choice_list[choice_index] = input(choice_labels[choice_index] + "\nEnter 'DONE' to exit.\n")
            ...
            if choice_list[choice_index] != "DONE":
                ...
                # Otherwise, raise an exception because of unintended behavior
                    ...
                    raise Exception("An unprogrammed datatype for hyperparameter " + choice_labels[choice_index] + " was specified. See choice_type variable.")
    ...
    hpo_trials_df = pd.DataFrame(hpo_trials, columns=choice_labels).sample(frac=1).reset_index(drop=True)
...
choice_list = [batch_size_choice, lr_choice, dropout_choice, weight_decay_beta1_choice, epsilon_choice, fc_choice]
choice_labels = ["Batch_Size", "lr", "Dropout_Rate", "Weight_Decay_Beta1", "Epsilon", "FC_Size"]
choice_type = [int, float, float, float, float, int]
hyperparameter_lists = [batch_size_list, lr_list, dropout_list, weight_decay_beta1_list, epsilon_list, fc_size]

hpo_trials_df = generate_grid_search_file(choice_list, choice_labels, choice_type, hyperparameter_lists)
print(hpo_trials_df)
